...
wp.customize.controlConstructor['maximo-google-fonts'] = wp.customize.Control.extend({
...
( function($) {
	...
	jQuery( document ).ready( function($) {
		...
		$('.google-fonts-list').on('change', function() {
			...
			$.each(bodyfontcontrol.googlefontslist[index].variants, function(val, text) {
				elementRegularWeight.append(
					$('<option></option>').val(text).html( maximo_readable_font(text) )
				);
		...
		function maximo_readable_font( text ) {
